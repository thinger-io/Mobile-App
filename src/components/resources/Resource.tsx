...
class ResourceComponent extends React.Component<Props> {
  renderButtons() {
    ...
    if (onUpdateClick !== undefined) {
      buttons.push(<Button text="Update" color={LIGHT_BLUE} isLoading={isFetching} onClick={onUpdateClick} />);
    }
    if (onChartClick !== undefined) {
      buttons.push(<Button text="Charts" color={LIGHT_PINK} onClick={onChartClick} />);
    }
    if (onPostClick !== undefined) {
      buttons.push(<Button color={LIGHT_GREEN} text="Post" isLoading={isFetching} onClick={onPostClick} />);
    }
    return buttons;
  }
