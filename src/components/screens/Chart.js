...
import {
  FlatList, View, AppState, Dimensions,
} from 'react-native';
...
class ChartScreen extends React.Component<Props, State> {
  ...
  componentWillUnmount() {
    ...
    if (this.state.refreshInterval) this.props.onFinish(this.state.refreshInterval);
  }

  handleStreaming() {
    const {
      device, resource, onInit, onRefresh,
    } = this.props;
    const delay = 1000;
  ...
  handleOnLabelClick(key) {
    const { selectedAttributes, onDeselectAttribute, onSelectAttribute } = this.props;
        ...
        tabs={[
          {
            ...
            icon: 'line-chart',
            active: type === 'Lines',
            onPress: () => this.setState(update(this.state, { type: { $set: 'Lines' } })),
          },
          {
            title: 'Pie',
            icon: 'pie-chart',
            active: type === 'Pie',
            onPress: () => this.setState(update(this.state, { type: { $set: 'Pie' } })),
          },
          {
            title: 'Bars',
            icon: 'bar-chart',
            active: type === 'Bars',
            onPress: () => this.setState(update(this.state, { type: { $set: 'Bars' } })),
          },
        ]}
      />
    ) : (
  ...
  renderChart() {
    const {
      data, orientation, streaming, onLockAttribute, onUnlockAttribute,
    } = this.props;
    const type = this.state.type;
    ...
    return (
      ...
      >
        ...
        {type === 'Bars' && typeof data === 'object' && (
          <Bars chartedAttributes={chartedAttributes} data={data} height={height} width={width} />
        )}
        {type === 'Pie' && typeof data === 'object' && (
          <Pie
            chartedAttributes={chartedAttributes}
            data={data}
  ...
  renderLabels() {
    const {
      orientation, selectedAttributes, lockedAttributes, data,
    } = this.props;
    const type = this.state.type;
...
const mapDispatchToProps = dispatch => ({
  ...
  onDeselectAttribute: (key, chart) => dispatch(deselectAttribute(key, chart)),
  onLockAttribute: (key, chart) => dispatch(lockAttribute(key, chart)),
  onUnlockAttribute: (key, chart) => dispatch(unlockAttribute(key, chart)),
  onRefresh: (device, resource) => dispatch(getResourceFromApi(device, resource)),
  onInit: (device, resource) => {
    dispatch(getResourceFromApi(device, resource)).then(({ value }) => {
      if (typeof value.out === 'number') {
        ...
        Object.keys(value.out).forEach(key => types.forEach((chart) => {
